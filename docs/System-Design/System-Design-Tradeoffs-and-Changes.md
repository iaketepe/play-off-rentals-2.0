...
- Step 2 was relatively straightforward. I started by creating a SQL schema in Supabase to define the tables for the store. For storing arcade machine images, I leveraged Supabase’s storage buckets, keeping image files separate from the database and storing their URLs in the tables.
